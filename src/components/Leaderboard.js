...
export default class Leaderboard extends Component {
  ...
  isLast(player) {
    const players = this.state.players;
    return (
      players.length > 0 && player.points === players[players.length - 1].points
    );
  }
    ...
    if (players.length > 0) {
      return (
        <div>
              ...
              {players.map(player => (
                <Table.Row key={player._id}>
                  <Table.Cell>
                    ...
                    {player.name}{" "}
                    {player.cheat ? (
                      <i aria-hidden="true" className="spy icon" />
                    ) : null}
                  </Table.Cell>
                  ...
                  <Table.Cell>
                    {Math.round((player.wins / player.games) * 10000) / 100}%
                  </Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          ...
          <p>
            <i aria-hidden="true" className="spy icon" /> Indicates possible
            cheating and/or use of performance enhancing drugs
          </p>
        </div>
      );
    } else {
